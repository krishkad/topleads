...
const Month = () => {
    ...
    return (
        <div className={cn('w-full grid grid-cols-7 gap-3', excludeDisabledWeek(month)?.lastRow ? "grid-rows-7" : "grid-rows-6")}>
            {dayHeader.map((day) => {
                return <div className={cn("w-full shadow-md aspect-video rounded-md", day.bg, day.text)} key={day.day}>
                ...
                return <Fragment key={i}>
                    {row.map((day: any, index: number) => {
                        return <Button
                            variant={'none'}
                            size={'none'}
                            className={cn(`w-full shadow-md aspect-video transition-all duration-100 rounded-md`, dayHeader[index].hoverBg, getCurrentDay(day) && dayHeader[index].active)}
                            key={index}
                            disabled={
                        ...
                        >
                            <div className="w-full h-full p-2 flex flex-col items-start">
                                ...
                                {getCurrentDay(day) && (<p className="font-medium text-sm">TODAY</p>)}
                            </div>
                        </Button>
                    })}
                </Fragment>
